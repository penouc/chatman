 ...
 * limitations under the License.
 */
const BASE_URL = "https://gemini.titi.li";
...
class RequestUrl {
  ...
  toString() {
    let url = `${BASE_URL}/${API_VERSION}/${this.model}:${this.task}?key=${this.apiKey}`;
    if (this.stream) {
      url += "?alt=sse";
    }
    return url;
...
async function makeRequest(url, body, requestOptions) {
  ...
  try {
    response = await fetch(
      url.toString(),
      Object.assign(Object.assign({}, buildFetchOptions(requestOptions)), {
        method: "POST",
        headers: {
